...
type DetailData = Awaited<ReturnType<typeof getMovieDetailData>>;
...
export default function MovieDetailClient({ movieId }: { movieId: number }) {
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          <div className="border-4" style={{ borderColor: "#000", background: "#fff" }}>
            <div className="p-8 text-center">
              <h1 className="text-2xl font-black uppercase text-black">No se pudo cargar la pelicula</h1>
              <p className="text-sm font-semibold mt-3" style={{ color: "#666" }}>
                Intenta recargar la pagina o vuelve a abrirla desde la lista.
              </p>
            </div>
          </div>
        </div>
  ...
  const { movie, director, topCast, similarMovies, backdropUrl, posterUrl, year, runtime } = data;
        ...
        <div style={{ border: "3px solid #000", background: "#fff" }}>
          <div className="relative h-48 md:h-64 overflow-hidden" style={{ borderBottom: "3px solid #000" }}>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
              {year && (
                <span className="inline-block text-xs font-black tracking-widest px-2 py-0.5 mb-2" style={{ background: "#ffbd3f", color: "#000" }}>
                  {year}
                </span>
              )}
              <h1 className="text-2xl md:text-3xl font-black text-white uppercase leading-tight">{movie.title}</h1>
            </div>
          </div>

          <div className="flex">
            <div className="flex-shrink-0 flex flex-col items-center gap-4 p-4" style={{ width: "160px", borderRight: "3px solid #000", background: "#fafafa" }}>
              {posterUrl && (
              ...
              <div>
                ...
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => {
                    ...
                    return (
                      <span key={i} className="text-base font-black" style={{ color: filled ? "#ffbd3f" : "#ddd" }}>
                        ★
                      </span>
                    );
                  })}
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {movie.genres.map((g, i) => (
                      <span key={g.id} className="text-xs font-black px-2.5 py-1 uppercase tracking-wide text-white" style={{ background: PALETTE[i % PALETTE.length] }}>
                        {g.name}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
                  {topCast.map((actor) => (
                    <div key={actor.id} className="flex flex-col items-center text-center">
                      <div className="w-12 h-12 overflow-hidden mb-1.5 flex-shrink-0" style={{ background: "#e0e0e0", border: "2px solid #000" }}>
                        {actor.profileUrl ? (
                          <Image src={actor.profileUrl} alt={actor.name} width={48} height={48} className="object-cover w-full h-full" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center font-black" style={{ color: "#999" }}>
                            ?
                          </div>
                        )}
                      </div>
          ...
          <div className="flex items-center justify-between px-6 py-3" style={{ borderTop: "3px solid #000", background: "#000" }}>
            <span className="font-black text-xs tracking-widest" style={{ color: "#ffbd3f" }}>
              VIO — #{movie.id}
            </span>
            <span className="font-black text-xs tracking-widest text-white opacity-40">★ NOT TRANSFERABLE ★</span>
            <span className="font-black text-xs tracking-widest" style={{ color: "#ffbd3f" }}>
              {year ?? "—"}
            </span>
          </div>
          ...
          <section className="mt-12">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-black uppercase tracking-widest text-black">Peliculas similares</h2>
              </div>
              <Link href={`/movie/${movie.id}/similar`} className="text-xs font-black tracking-widest uppercase px-4 py-2 transition-opacity hover:opacity-80" style={{ background: "#000", color: "#ffbd3f" }}>
                VER MAS
              </Link>
            </div>
